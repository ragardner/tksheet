...
class TopLeftRectangle(tk.Canvas):
    def __init__(self, *_, **kwargs):
        super().__init__(kwargs["parentframe"], background=kwargs["top_left_bg"], highlightthickness=0)
        ...
        self.MT: MainTable = kwargs["main_canvas"]
        self.RI: RowIndex = kwargs["row_index_canvas"]
        self.CH: ColumnHeaders = kwargs["header_canvas"]
        try:
    ...
    def set_dimensions(self, new_w: ScreenUnits | None = None, new_h: ScreenUnits | None = None):
        try:
            if new_w:
    ...
    def b1_motion(self, event: tk.Event[Self] | None = None):
        self.focus_set()
