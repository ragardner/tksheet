...
if TYPE_CHECKING:
    ...
    from tksheet.types import ScreenUnits
...
class ColumnHeaders(tk.Canvas):
    ...
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(kwargs["parentframe"], background=kwargs["header_bg"], highlightthickness=0)
        self.parentframe: MainTable = kwargs["parentframe"]
        ...
        self.ch_extra_begin_drag_drop_func: Callable | None = None
        self.ch_extra_end_drag_drop_func: Callable | None = None
        ...
        self.column_width_resize_func: Callable | None = None
    ...
    def mousewheel(self, event: tk.Event[Self]):
        maxlines = 0
    ...
    def set_height(self, new_height, set_TL: bool = False) -> None:
    ...
    def enable_bindings(self, binding: str) -> None:
    ...
    def disable_bindings(self, binding: str) -> None:
        if binding == "column_width_resize":
            self.width_resizing_enabled = False
        if binding == "column_height_resize":
            self.height_resizing_enabled = False
    ...
    def rc(self, event: tk.Event[Self]):
            ...
            if c < len(self.MT.col_positions) - 1:
                ...
                if not c_selected and self.col_selection_enabled:
                    ...
                    self.dragged_col = DraggedRowColumn(
                        dragged=c, to_move=get_seq_without_gaps_at_index(sorted(self.MT.get_selected_cols()), c)
                    )
        elif not self.MT.ctrl_select_enabled:
            self.b1_press(event)

    def ctrl_shift_b1_press(self, event: tk.Event[Self]):
        self.mouseclick_outside_editor_or_dropdown_all_canvases()
        x = event.x
    ...
    def shift_b1_press(self, event: tk.Event[Self]):
        self.mouseclick_outside_editor_or_dropdown_all_canvases()
        x = event.x
        c = self.MT.identify_col(x=x)
    ...
    def create_resize_line(self, x1: float, y1: float, x2: float, y2: float, width, fill, tag) -> None:
    ...
    def b1_press(self, event: tk.Event[Self]) -> None:
        ...
        if self.width_resizing_enabled and self.rsz_w is not None:
            ...
            self.create_resize_line(
                line2x, 0, line2x, self.current_height, width=1, fill=self.resizing_line_fg, tag="rwl2"
            )
    ...
    def b1_motion(self, event: tk.Event[Self]) -> None:
        x1, y1, x2, y2 = self.MT.get_canvas_visible_area()
        if self.width_resizing_enabled and self.rsz_w is not None and self.currently_resizing_width:
            ...
            if x > 0 and x < self.MT.col_positions[-1]:
                self.show_drag_and_drop_indicators(
                    self.drag_and_drop_motion(event), y1, y2, self.dragged_col.to_move[0], self.dragged_col.to_move[-1]
                )
        elif (
    ...
    def ctrl_b1_motion(self, event: tk.Event[Self]) -> None:
        x1, y1, x2, y2 = self.MT.get_canvas_visible_area()
        if (
            self.drag_and_drop_enabled
            and self.col_selection_enabled
    ...
    def show_drag_and_drop_indicators(self, xpos, y1: float, y2: float, start_col: int, end_col: int) -> None:
    ...
    def delete_all_resize_and_ctrl_lines(self, ctrl_lines: bool = True) -> None:
        self.delete_resize_lines()
        self.MT.delete_resize_lines()
        if ctrl_lines:
            self.MT.delete_ctrl_outlines()

    def scroll_if_event_offscreen(self, event: tk.Event[Self]) -> bool:
        xcheck = self.xview()
        need_redraw = False
    ...
    def event_over_dropdown(self, c: int, datacn, event: tk.Event[Self], canvasx) -> bool:
        if (
    ...
    def event_over_checkbox(self, c: int, datacn, event: tk.Event[Self], canvasx):
        if (
            event.y < self.MT.header_txt_h + 5
                ...
                if self.ch_extra_begin_drag_drop_func is not None:
                    try:
                        self.ch_extra_begin_drag_drop_func(
                            BeginDragDropEvent("begin_column_header_drag_drop", tuple(orig_selected), int(c))
                        )
                    except Exception:
                        extra_func_success = False
                if extra_func_success:
                    new_selected, dispset = self.MT.move_columns_adjust_options_dict(
                        c, rm1start, totalcols, move_data=self.column_drag_and_drop_perform
                    )
                    ...
                    if self.ch_extra_end_drag_drop_func is not None:
                        self.ch_extra_end_drag_drop_func(
                            EndDragDropEvent("end_column_header_drag_drop", orig_selected, new_selected, int(c))
                        )
    ...
    def readonly_header(self, columns: int | Sequence[int] = [], readonly: bool = True) -> None:
    ...
    def toggle_select_col(
        ...
        if add_selection:
            if self.MT.col_selected(column):
                ...
                self.add_selection(
                    c=column, redraw=redraw, run_binding_func=run_binding_func, set_as_current=set_as_current
                )
        else:
            if self.MT.col_selected(column):
                self.MT.deselect(c=column, redraw=redraw)
    ...
    def select_col(self, c: int, redraw: bool = False) -> None:
        ...
        if self.selection_binding_func is not None:
            self.selection_binding_func(SelectColumnEvent("select_column", c))

    def add_selection(self, c: int, redraw: bool = False, run_binding_func=True, set_as_current=True):
        c = int(c)  # ! WHY?
        if set_as_current:
            self.MT.set_currently_selected(0, c, type_="column")
        self.MT.create_selected(0, c, len(self.MT.row_positions) - 1, c + 1, "columns")
    ...
    def set_height_of_header_to_text(self, text: str | None = None):
        if (
            text is None
    ...
    def set_col_width(
        ...
        qtxtm = self.MT.txt_measure_canvas_text
        qtxth = self.MT.txt_h
        qclop = self.MT.cell_options  # ! Neeed?
    ...
    def set_width_of_all_cols(self, width=None, only_set_if_too_small: bool = False, recreate: bool = True) -> None:
        if width is None:
            if self.MT.all_columns_displayed:
            ...
            self.MT.col_positions = list(
                accumulate(
                    chain(
                        ...
                        (
                            self.set_col_width(
                                cn, only_set_if_too_small=only_set_if_too_small, recreate=False, return_new_width=True
                            )
                            for cn in iterable
                        ),
                    )
    ...
    def align_cells(self, columns: int | Sequence[int] = [], align: str = "global") -> None:
        if isinstance(columns, int):
            cols = [columns]
        else:
            cols = columns
    ...
    def redraw_gridline(self, points, fill, width, tag):
        if self.hidd_grid:
            ...
            if sh:
                ...
                self.itemconfig(
                    t, fill=fill, width=width, tag=tag, capstyle=tk.BUTT, joinstyle=tk.ROUND, state="normal"
                )
    ...
    def redraw_grid_and_text(
        self, last_col_line_pos, scrollpos_left, x_stop, start_col, end_col, scrollpos_right, col_pos_exists
    ):
        try:
            self.configure(scrollregion=(0, 0, last_col_line_pos + self.MT.empty_horizontal, self.current_height))
        except Exception:
            return
    ...
    def open_text_editor(
        self, event=None, c=0, text=None, state="normal", see=True, set_data_on_close=True, binding=None, dropdown=False
    ):
        text = None
        extra_func_key = "??"
            ...
            if (
                ...
                if new_height != curr_height:
                    ...
                    if kwargs:
                        ...
                        self.coords(kwargs["canvas_id"], self.MT.col_positions[c], new_height - 1)
    ...
    def set_cell_data_undo(
        self, c=0, datacn=None, value="", cell_resize=True, undo=True, redraw: bool = True, check_input_valid=True
    ):
        if datacn is None:
            datacn = c if self.MT.all_columns_displayed else self.MT.displayed_columns[c]
        if isinstance(self.MT._headers, int):
    ...
    # internal event use
    def click_checkbox(self, c, datacn=None, undo=True, redraw: bool = True):
        if datacn is None:
            datacn = c if self.MT.all_columns_displayed else self.MT.displayed_columns[c]
        kwargs = self.get_cell_kwargs(datacn, key="checkbox")
    ...
    def open_dropdown_window(self, c, datacn=None, event: tk.Event[Self] | None = None):
        self.destroy_text_editor("Escape")
        self.destroy_opened_dropdown_window()
        if datacn is None:
            datacn = c if self.MT.all_columns_displayed else self.MT.displayed_columns[c]
    ...
    def close_dropdown_window(self, c=None, selection=None, redraw: bool = True):
    ...
    def get_cell_kwargs(self, datacn, key="dropdown", cell: bool = True, entire: bool = True):
