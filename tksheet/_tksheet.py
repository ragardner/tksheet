...
class Sheet(tk.Frame):
    ...
    def index_align(self, align: str = None, redraw=True):
    ...
    def set_header_data(self, value, c=None, redraw=True):
        if c is None:
    ...
    def set_index_data(self, value, r=None, redraw=True):
        if r is None:
            if not isinstance(value, int) and not is_iterable(value):
                raise ValueError(("Argument 'value' must be non-string iterable or int, " f"not {type(value)} type."))
            if not isinstance(value, list) and not isinstance(value, int):
    ...
    def set_cell_data(self, r, c, value="", redraw=True, keep_formatting=True):
