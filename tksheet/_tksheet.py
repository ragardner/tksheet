...
if TYPE_CHECKING:
    ...
    from tksheet.types import Align, Binding, Canvas, Font, ScreenUnits
...
class Sheet(tk.Frame):
    def __init__(
        ...
        from_clipboard_delimiters: Sequence[str] = ["\t"],
        ...
        data_reference: list[...] | None = None,
        data: list[...] | None = None,
        startup_select: tuple[...]
        | None = None,  # either (start row, end row, "rows"), (start column, end column, "rows") or (cells start row, cells start column, cells end row, cells end column, "cells")
        startup_focus: bool = True,
        total_columns: int | None = None,
        total_rows: int | None = None,
        column_width: int = 120,
        header_height: str | int = "1",  # str or int
        max_column_width: str | int = "inf",  # str or int
        max_row_height: str | int = "inf",  # str or int
        max_header_height: str | int = "inf",  # str or int
        max_index_width: str | int = "inf",  # str or int
        row_index: list | None = None,
        index: list | None = None,
        after_redraw_time_ms: int = 20,
        row_index_width: int | None = None,
        auto_resize_default_row_index: bool = True,
        set_all_heights_and_widths: bool = False,
        row_height: str | int = "1",  # str or int
        font: Font = get_font(),
        header_font: Font = get_heading_font(),
            ...
            try:
                if startup_select[-1] == "cells":
                    ...
                    self.MT.create_selected(
                        startup_select[0], 0, startup_select[1], len(self.MT.col_positions) - 1, "rows"
                    )
                    ...
                    self.MT.create_selected(
                        0, startup_select[0], len(self.MT.row_positions) - 1, startup_select[1], "columns"
                    )
    ...
    def show(self, canvas: Canvas = "all") -> None:
        if canvas == "all":
            self.hide()
            self.TL.grid(row=0, column=0)
            self.RI.grid(row=1, column=0, sticky="nswe")
    ...
    def hide(self, canvas: Canvas = "all") -> None:
        if canvas.lower() == "all":
            self.TL.grid_forget()
            self.RI.grid_forget()
    ...
    def height_and_width(self, height: ScreenUnits | None = None, width: ScreenUnits | None = None) -> None:
        if width is not None or height is not None:
    ...
    def focus_set(self, canvas: Literal["table", "header", "index", "topleft"] = "table") -> None:
        if canvas == "table":
            self.MT.focus_set()
        elif canvas == "header":
            self.CH.focus_set()
    ...
    def popup_menu_del_command(self, label: str | None = None):
        if label is None:
    ...
    def extra_bindings(self, bindings: Sequence[Binding], func: Callable = None):
    ...
    def bind_event(self, sequence, func: Callable[..., object], add: bool | Literal["", "+"] | None = None):
        widget = self

        def _substitute(*args: Any) -> Sequence[Any]:
            e: Any = lambda: None
            e.data = eval(args[0])
            e.widget = widget
            return (e,)

        funcid = widget.register(func, _substitute, needcleanup=1)
        cmd = '{0}if {{"[{1} %d]" == "break"}} break\n'.format("+" if add else "", funcid)
        widget.tk.call("bind", widget._w, sequence, cmd)

    def bind(
        self, binding: str, func: Callable[[tk.Event[MainTable]], object], add: bool | Literal["", "+"] | None = None
    ) -> None:
    ...
    def enable_bindings(self, *bindings: str) -> None:
        self.MT.enable_bindings(bindings)

    def disable_bindings(self, *bindings: str) -> None:
    ...
    def cell_edit_binding(self, enable: bool = False, keys=[]) -> None:
        self.MT.bind_cell_edit(enable, keys=[])

    def identify_region(self, event: tk.Event[Self]) -> Literal["table", "index", "header", "top left"] | None:
    ...
    def identify_row(self, event: tk.Event[Self], exclude_index: bool = False, allow_end: bool = True):
        ev_w = event.widget
        if ev_w == self.MT:
    ...
    def identify_column(self, event: tk.Event[Self], exclude_header: bool = False, allow_end: bool = True) -> None:
        ev_w = event.widget
        if ev_w == self.MT:
    ...
    def get_example_canvas_column_widths(self, total_cols=None) -> list[int]:
    ...
    def get_example_canvas_row_heights(self, total_rows=None) -> list[int]:
    ...
    def get_column_widths(self, canvas_positions: bool = False) -> list[int]:
        ...
        return [
            int(b - a)
            for a, b in zip(self.MT.col_positions, islice(self.MT.col_positions, 1, len(self.MT.col_positions)))
        ]
    ...
    def get_row_heights(self, canvas_positions: bool = False) -> list[int]:
        ...
        return [
            int(b - a)
            for a, b in zip(self.MT.row_positions, islice(self.MT.row_positions, 1, len(self.MT.row_positions)))
        ]
    ...
    def set_all_column_widths(
        self, width=None, only_set_if_too_small=False, redraw: bool = True, recreate_selection_boxes=True
    ):
        self.CH.set_width_of_all_cols(
            width=width, only_set_if_too_small=only_set_if_too_small, recreate=recreate_selection_boxes
        )
        self.set_refresh_timer(redraw)

    def column_width(self, column=None, width=None, only_set_if_too_small=False, redraw: bool = True):
        if column == "all":
            if width == "default":
    ...
    def set_column_widths(
        self, column_widths=None, canvas_positions: bool = False, reset: bool = False, verify: bool = False
    ):
        cwx = None
        if reset:
            self.MT.reset_col_positions()
    ...
    def set_all_row_heights(
        ...
        self.RI.set_height_of_all_rows(
            height=height, only_set_if_too_small=only_set_if_too_small, recreate=recreate_selection_boxes
        )
        self.set_refresh_timer(redraw)

    def set_cell_size_to_text(self, row: int, column: int, only_set_if_too_small: bool = False, redraw: bool = True):
    ...
    def row_height(self, row=None, height=None, only_set_if_too_small=False, redraw: bool = True):
        if row == "all":
            if height == "default":
    ...
    def default_row_height(self, height: ScreenUnits | None = None):
        if height is not None:
    ...
    def default_header_height(self, height: ScreenUnits | None = None):
        if height is not None:
    ...
    def delete_row_position(self, idx: int, deselect_all: bool = False) -> None:
        self.MT.del_row_position(idx=idx, deselect_all=deselect_all)

    def delete_row(self, idx=0, deselect_all=False, redraw: bool = True) -> None:
        self.delete_rows(rows={idx}, deselect_all=deselect_all, redraw=False)
        self.set_refresh_timer(redraw)

    def delete_rows(self, rows: set = set(), deselect_all=False, redraw: bool = True):
        if deselect_all:
            ...
            self.set_row_heights(
                row_heights=(
                    ...
                    for r, h in enumerate(
                        int(b - a)
                        for a, b in zip(
                            self.MT.row_positions, islice(self.MT.row_positions, 1, len(self.MT.row_positions))
                        )
                    )
                    if r not in to_del
                )
        ...
        self.MT.cell_options = {
            (r if not bisect.bisect_left(to_bis, r) else r - bisect.bisect_left(to_bis, r), c): v
    ...
    def insert_row_position(self, idx="end", height=None, deselect_all=False, redraw: bool = False):
        self.MT.insert_row_position(idx=idx, height=height, deselect_all=deselect_all)
        self.set_refresh_timer(redraw)

    def insert_row_positions(self, idx="end", heights=None, deselect_all=False, redraw: bool = False):
    ...
    def delete_column(self, idx=0, deselect_all=False, redraw: bool = True):
        self.delete_columns(columns={idx}, deselect_all=deselect_all, redraw=False)
        self.set_refresh_timer(redraw)

    def delete_columns(self, columns: set = set(), deselect_all=False, redraw: bool = True):
        if deselect_all:
            self.deselect("all", redraw=False)
            ...
            self.set_column_widths(
                column_widths=(
                    ...
                    for c, w in enumerate(
                        int(b - a)
                        for a, b in zip(
                            self.MT.col_positions, islice(self.MT.col_positions, 1, len(self.MT.col_positions))
                        )
                    )
                    if c not in to_del
                )
        ...
        self.MT.cell_options = {
            (r, c if not bisect.bisect_left(to_bis, c) else c - bisect.bisect_left(to_bis, c)): v
            for (r, c), v in self.MT.cell_options.items()
            if c not in to_del
        }
        self.MT.col_options = {
    ...
    def insert_column_position(self, idx="end", width=None, deselect_all=False, redraw: bool = False):
        self.MT.insert_col_position(idx=idx, width=width, deselect_all=deselect_all)
        self.set_refresh_timer(redraw)

    def insert_column_positions(self, idx="end", widths=None, deselect_all=False, redraw: bool = False):
    ...
    def move_columns(
        ...
        new_selected, dispset = self.MT.move_columns_adjust_options_dict(
            moveto, to_move_min, number_of_columns, move_data, create_selections, index_type=index_type.lower()
        )
        self.set_refresh_timer(redraw)
    ...
    def move_rows(
        ...
        new_selected, dispset = self.MT.move_rows_adjust_options_dict(
            moveto, to_move_min, number_of_rows, move_data, create_selections, index_type=index_type.lower()
        )
        self.set_refresh_timer(redraw)
        return new_selected, dispset

    # works on currently selected box
    def open_cell(self, ignore_existing_editor: bool = True) -> None:
    ...
    def get_text_editor_widget(self, event: tk.Event[Self] | None = None):
        try:
    ...
    def add_cell_selection(self, row, column, redraw: bool = True, run_binding_func=True, set_as_current=True):
        self.MT.add_selection(
            r=row, c=column, redraw=False, run_binding_func=run_binding_func, set_as_current=set_as_current
        )
        self.set_refresh_timer(redraw)

    def add_row_selection(self, row, redraw: bool = True, run_binding_func=True, set_as_current=True):
        self.RI.add_selection(r=row, redraw=False, run_binding_func=run_binding_func, set_as_current=set_as_current)
        self.set_refresh_timer(redraw)

    def add_column_selection(self, column, redraw: bool = True, run_binding_func=True, set_as_current=True):
    ...
    def toggle_select_column(
        self, column, add_selection=True, redraw: bool = True, run_binding_func=True, set_as_current=True
    ):
    ...
    def deselect(self, row=None, column=None, cell=None, redraw: bool = True):
    ...
    def set_currently_selected(self, row, column, type_="cell", selection_binding=True):
        self.MT.set_currently_selected(
            r=row, c=column, type_=type_, inside=True if self.MT.cell_selected(row, column) else False
        )
    ...
    def anything_selected(self, exclude_columns=False, exclude_rows=False, exclude_cells=False):
        if self.MT.anything_selected(
            exclude_columns=exclude_columns, exclude_rows=exclude_rows, exclude_cells=exclude_cells
        ):
            return True
        return False
    ...
    def readonly_rows(self, rows=[], readonly=True, redraw: bool = False):
    ...
    def readonly_columns(self, columns=[], readonly=True, redraw: bool = False):
    ...
    def readonly_cells(self, row=0, column=0, cells=[], readonly=True, redraw: bool = False):
        if not readonly:
            if cells:
                for r, c in cells:
    ...
    def readonly_header(self, columns=[], readonly=True, redraw: bool = False):
        self.CH.readonly_header(columns=columns, readonly=readonly)
        self.set_refresh_timer(redraw)

    def readonly_index(self, rows=[], readonly=True, redraw: bool = False):
    ...
    def highlight_rows(
        self, rows=[], bg=None, fg=None, highlight_index=True, redraw: bool = True, end_of_screen=False, overwrite=True
    ):
    ...
    def highlight_columns(
        self, columns=[], bg=None, fg=None, highlight_header=True, redraw: bool = True, overwrite=True
    ):
        if bg is None and fg is None:
            return
    ...
    def highlight_cells(
        self, row=0, column=0, cells=[], canvas="table", bg=None, fg=None, redraw: bool = True, overwrite=True
    ):
        if bg is None and fg is None:
            return
        if canvas == "table":
    ...
    def dehighlight_cells(self, row=0, column=0, cells=[], canvas="table", all_=False, redraw: bool = True):
    ...
    def align_rows(
        self, rows=[], align="global", align_index=False, redraw: bool = True
        ...
        if align == "global" or self.convert_align(align):
            if isinstance(rows, dict):
                ...
                self.MT.align_rows(
                    rows=rows, align=align if align == "global" else self.convert_align(align), align_index=align_index
                )
        self.set_refresh_timer(redraw)

    def align_columns(
        self, columns=[], align="global", align_header=False, redraw: bool = True
    ):  # "center", "w", "e" or "global"
        if align == "global" or self.convert_align(align):
    ...
    def align_cells(
        self, row=0, column=0, cells=[], align="global", redraw: bool = True
    ):  # "center", "w", "e" or "global"
        if align == "global" or self.convert_align(align):
            if isinstance(cells, dict):
                ...
                self.MT.align_cells(
                    row=row, column=column, cells=cells, align=align if align == "global" else self.convert_align(align)
                )
        self.set_refresh_timer(redraw)
    ...
    def change_theme(self, theme="light blue", redraw: bool = True):
    ...
    def get_row_data(
        self, r, get_displayed=False, get_index=False, get_index_displayed=True, only_columns=None, **kwargs
    ):
        if only_columns is not None:
            if isinstance(only_columns, int):
                only_columns = (only_columns,)
    ...
    def get_column_data(
        self, c, get_displayed=False, get_header=False, get_header_displayed=True, only_rows=None, **kwargs
    ):
        if only_rows is not None:
            if isinstance(only_rows, int):
                only_rows = (only_rows,)
    ...
    def set_sheet_data(
        ...
        return self.MT.data_reference(
            data, reset_col_positions, reset_row_positions, redraw, return_id=False, keep_formatting=keep_formatting
        )

    def set_cell_data(self, r, c, value="", redraw: bool = False, keep_formatting=True):
    ...
    def set_row_data(self, r, values=tuple(), add_columns=True, redraw: bool = False, keep_formatting=True):
        if r >= len(self.MT.data):
    ...
    def set_column_data(self, c, values=tuple(), add_rows=True, redraw: bool = False, keep_formatting=True):
        if not keep_formatting:
    ...
    def insert_rows(
        ...
        if add_columns:
            ...
            if data_max_cols > total_cols:
                ...
                data[:] = [
                    r + self.MT.get_empty_row_seq(datarn + i, end=total_cols, start=data_max_cols, r_ops=False)
        ...
        if mod_row_positions:
            self.MT.insert_row_positions(
                idx=inspos, heights=len(data) if heights is None else heights, deselect_all=deselect_all
            )
    ...
    def hide_rows(self, rows=set(), redraw: bool = True, deselect_all=True):
        if isinstance(rows, int):
            _rows = {rows}
    ...
    # uses displayed indexes
    def hide_columns(self, columns=set(), redraw: bool = True, deselect_all=True):
        if isinstance(columns, int):
            _columns = {columns}
    ...
    def format_cell(
        ...
        **kwargs,
        ...
        if isinstance(r, str) and r.lower() == "all" and isinstance(c, int):
            for r_ in range(self.MT.total_data_rows()):
                self.MT.format_cell(
                    datarn=r_, datacn=c, **{"formatter": formatter_class, **formatter_options, **kwargs}
                )
        elif isinstance(c, str) and c.lower() == "all" and isinstance(r, int):
            for c_ in range(self.MT.total_data_cols()):
                self.MT.format_cell(
                    datarn=r, datacn=c_, **{"formatter": formatter_class, **formatter_options, **kwargs}
                )
        elif isinstance(r, str) and r.lower() == "all" and isinstance(c, str) and c.lower() == "all":
            for r_ in range(self.MT.total_data_rows()):
                for c_ in range(self.MT.total_data_cols()):
                    self.MT.format_cell(
                        datarn=r_, datacn=c_, **{"formatter": formatter_class, **formatter_options, **kwargs}
                    )
        else:
...
class Sheet_Dropdown(Sheet):
    ...
    def search_and_see(self, event: tk.Event[Self] | None = None):
    ...
    def mouse_motion(self, event: tk.Event[Self] | None = None):
    ...
    def b1(self, event: tk.Event[Self] | None = None):
        if event is None:
            row = None
    ...
    def values(self, values=[], redraw: bool = True):
        self.set_sheet_data(
            [[v] for v in values], reset_col_positions=False, reset_row_positions=False, redraw=False, verify=False
        )
